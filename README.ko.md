...
**v1.0.0** — 실제 GitHub URL 경로로 설치 검증 완료: 24개 파일 fetch, 재시도 0회, 최종 `verify` green. [English README](README.md)
...
설치 자체가 첫 세션 카드가 됩니다 — 에이전트가 작업을 마치기 전에 설치 세션이 아카이브됩니다. 설치 1분 뒤 에이전트에게 *"아카이브가 왜 이렇게 구성됐지?"*라고 물어보세요 — 기억이 아니라 카드에서 답합니다. 3주 뒤, 다른 사람의 에이전트도 같은 질문에 같은 답을 합니다.
...
- **질문에 답하는 세션 아카이브** — "사람 1 + AI 1" 세션마다 결정·이유·변경·영향 파일이 담긴 결과 카드가 커밋됩니다. 대화 원본은 로컬에만 남고 git-ignore되며 카드와 SHA-256으로 연결됩니다. *"X는 어떻게 됐지?"*라고 묻거나 `node scripts/hnk.mjs report`로 다이제스트를 받으세요.
