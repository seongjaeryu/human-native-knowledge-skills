...
| --- | --- | --- |
| add | `node scripts/hnk.mjs visuals add <file> [--domain <domain>] [--topic <topic>] [--alt <text>]` | **moves** the source file under `files/` per §2 (moved, not copied — the payload lives in one place), computes `sha256` and `bytes`, appends the entry. **`alt` is required**: a missing `--alt` is a hard error — supply the description on the command line |
